...
regressions_df_ssp = DataFrame(
    ...
    intercept = missing,
    estimate = coef(combined_model_0_ssp),
    stderror_int = missing,
    stderror_est = stderror(combined_model_0_ssp),
    confint_99_low = confint_99_0_ssp[1],
    confint_99_high = confint_99_0_ssp[2],
    intercept_wo_0 = coef(combined_model_ssp)[1] + AMOC_pi_ssp * coef(combined_model_ssp)[2]
)

regressions_df_pct = DataFrame(
    name="combined_model_0_pct_$(running_window)yrs",
    intercept = missing,
    estimate = coef(combined_model_0_pct),
    stderror_int = missing,
    stderror_est = stderror(combined_model_0_pct),
    confint_99_low = confint_99_0_pct[1],
    confint_99_high = confint_99_0_pct[2],
    intercept_wo_0 = coef(combined_model_pct)[1] + AMOC_pi_pct * coef(combined_model_pct)[2]
)

regressions_df_ssp_wo_0 = DataFrame(
    name="combined_model_ssp_$(running_window)yrs",
    intercept = coef(combined_model_ssp)[1],
    estimate = coef(combined_model_ssp)[2],
    stderror_int = stderror(combined_model_ssp)[1],
    stderror_est = stderror(combined_model_ssp)[2],
    confint_99_low = confint_99_ssp[2, 1],
    confint_99_high = confint_99_ssp[2, 2],
    intercept_wo_0 = coef(combined_model_ssp)[1] + AMOC_pi_ssp * coef(combined_model_ssp)[2]
)

regressions_df_pct_wo_0 = DataFrame(
    name="combined_model_pct_$(running_window)yrs",
    intercept = coef(combined_model_pct)[1],
    estimate = coef(combined_model_pct)[2],
    stderror_int = stderror(combined_model_pct)[1],
    stderror_est = stderror(combined_model_pct)[2],
    confint_99_low = confint_99_pct[2, 1],
    confint_99_high = confint_99_pct[2, 2],
    intercept_wo_0 = coef(combined_model_pct)[1] + AMOC_pi_pct * coef(combined_model_pct)[2]
)
...
fig.text(-0.05, 0.5, latexstring("Carbon flux change \$ \\Delta F\\ \\left[\\frac{\\mathrm{PgC}}{\\mathrm{yr}}\\right]\$"), va="center", rotation="vertical")

axs[1].text(0.45, 0.95, "SSP2-4.5 \$\\mathbf{CO_2}\$ increase", transform=axs[1].transAxes, fontsize=15, weight="bold", ha="center")
axs[0].text(0.45, 0.95, "1% per year \$\\mathbf{CO_2}\$ increase", transform=axs[0].transAxes, fontsize=15, weight="bold", ha="center")
...
axs[1].annotate(latexstring("\$AMOC_\\text{0,ssp245}\$"), xy=(AMOC_pi_ssp, -0.41), xytext=(0, -32), textcoords="offset points", fontsize=12, arrowprops=Dict("facecolor"=>"dimgrey", "edgecolor"=>"dimgrey", "width"=>1, "headwidth"=>6, "headlength"=> 8, "linewidth"=>.01), horizontalalignment="center", verticalalignment="center", color="dimgrey", clip_on=false, transform=axs[1].get_yaxis_transform())
